...
class TestBaseModel(unittest.TestCase):
    ...
    def test_if_BaseModel_instance_has_id(self):
        """
        Test that instance has an id assigned on initialization
        """
        idassigned = BaseModel()
        self.assertTrue(hasattr(idassigned, "id"))

    def test_str_representation(self):
        """
        Test if the string representation is appropriate
        """
        stringcheck = BaseModel()
        self.assertEqual(str(stringcheck),
                         "[BaseModel] ({}) {}".format(stringcheck.id, stringcheck.__dict__))

    def test_ids_is_unique(self):
        """
        Test if id is generated randomly and uniquely
        """
        b1 = BaseModel()
        b2 = BaseModel()
    ...
    def test_type_of_id_is_str(self):
        """
        Test that id generated is a str object
        """
        strobj = BaseModel()
        self.assertTrue(type(strobj.id) is str)

    def test_created_at_is_datetime(self):
        """
        Test that the attribute 'created_at' is a datetime object
        """
        created_att = BaseModel()
        self.assertTrue(type(created_att.created_at) is datetime)
